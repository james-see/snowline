...
- **Course props** (`Props.ts`): trees and rocks are `InstancedMesh`; nearest-to-path kept within caps; only near instances cast shadows.
- **VFX** (`VfxModule.ts`): integrates/draws within snow/spray budgets via `setDrawRange`.
